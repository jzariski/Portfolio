...
OUT_PATH = "2025_05_09.out"   # .out file
...
def make_features_and_targets(data: np.ndarray):
    ...
    # Low-order geometric horizon transform features
    sin_el_geom = (
...
fig = plt.figure(figsize=(7, 6))
ax = fig.add_subplot(111)
ax.plot(az_abs, az_cdf, label="Az abs error")
ax.plot(el_abs, el_cdf, label="El abs error")
...
ax.grid(False)
plt.tight_layout()
plt.savefig("cdf_errors.png", dpi=200)
...
ax.hist(np.log10(abs(el_err)), bins=1000, density=True, cumulative=True, histtype='step', label='Error in Predicting Offset RA')
